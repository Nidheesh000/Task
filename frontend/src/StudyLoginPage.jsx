...
function StudyLoginPage(props) {
    ...
    const handleLogin = async () => {
        try {
            const response = await axios.post('https://task-55yk.onrender.com/login', { username, password });
    ...
    const handleRegister = async () => {
        try {
            const response = await axios.post('https://task-55yk.onrender.com/register', { username, password });
